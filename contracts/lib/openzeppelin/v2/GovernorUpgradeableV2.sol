    ...
    IERC1155ReceiverUpgradeable
{
    ...
    function state(uint256 proposalId) public view virtual override returns (ProposalState) {
        ...
        if (!TFHE.decrypt(_quorumReached(proposalId)) || !TFHE.decrypt(_voteSucceeded(proposalId))) {
    ...
    function castVote(uint256 proposalId, uint8 support) public virtual override returns (uint256) {
        ...
        return _castVote(proposalId, voter, TFHE.asEuint8(support), "", "");
    }
    ...
    function castVoteWithReason(
        ...
        address voter = _msgSender();
        return _castVote(proposalId, voter, TFHE.asEuint8(support), reason, "");
    }
    ...
    function castVoteWithReasonAndParams(
        ...
        address voter = _msgSender();
        return _castVote(proposalId, voter, TFHE.asEuint8(support), reason, params);
    }
    ...
    function _castVote(
        ...
        _countVote(proposalId, account, encryptedSupport, weight, params);

        if (params.length == 0) {
            emit VoteCast(account, proposalId, encryptedSupport, weight, reason);
        } else {
            emit VoteCastWithParams(account, proposalId, encryptedSupport, weight, reason, params);
        }
