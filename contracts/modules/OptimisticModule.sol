...
contract OptimisticModule_SocialSignalling is VotingModule {
    ...
    function propose(uint256 proposalId, bytes memory proposalData, bytes32 descriptionHash) external override {
        ...
        IProposalTypesConfigurator proposalConfigurator = IProposalTypesConfigurator(
            IAgoraGovernor(msg.sender).PROPOSAL_TYPES_CONFIGURATOR()
        );
        IProposalTypesConfigurator.ProposalType memory proposalType = proposalConfigurator.proposalTypes(
            proposalTypeId
        );
    ...
    function _formatExecuteParams(
        uint256,
        bytes memory
    ) public pure override returns (address[] memory targets, uint256[] memory values, bytes[] memory calldatas) {
