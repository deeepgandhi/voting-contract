...
import { VotingModule } from "contracts/modules/VotingModule.sol";
...
contract ApprovalVotingModule is VotingModule {
    ...
    mapping(uint256 proposalId => mapping(address account => EnumerableSetUpgradeable.UintSet votes))
        private accountVotesSet;
    ...
    function propose(uint256 proposalId, bytes memory proposalData, bytes32 descriptionHash) external override {
        ...
        (ProposalOption[] memory proposalOptions, ProposalSettings memory proposalSettings) = abi.decode(
            proposalData,
            (ProposalOption[], ProposalSettings)
        );
        ...
        proposals[proposalId].optionVotes = new euint32[](optionsLength);
    }
    ...
    function _countVote(
        ...
        if (TFHE.decrypt(TFHE.eq(support, TFHE.asEuint8(uint8(VoteType.For))))) {
            if (weight != 0) {
    ...
    function _formatExecuteParams(
    ...
    ) public override returns (address[] memory targets, uint256[] memory values, bytes[] memory calldatas) {
        _onlyGovernor();
        (ProposalOption[] memory options, ProposalSettings memory settings) = abi.decode(
            proposalData,
            (ProposalOption[], ProposalSettings)
        );
        ...
        (euint32[] memory sortedOptionVotes, ProposalOption[] memory sortedOptions) = _sortOptions(
            proposals[proposalId].optionVotes,
            options
        );

        (uint256 executeParamsLength, uint256 succeededOptionsLength) = _countOptions(
            sortedOptions,
            sortedOptionVotes,
            settings
        );
...

        {
            ...
            for (uint256 i; i < succeededOptionsLength; ) {
                ...
                for (n = 0; n < option.targets.length; ) {
                    ...
                    unchecked {
                        executeParams[executeParamsLength + n] = ExecuteParams(
                            option.targets[n],
                            option.values[n],
                            option.calldatas[n]
                        );
                ...
                if (settings.budgetAmount != 0) {
                    ...
                    if (settings.budgetToken != address(0)) {
                        if (option.budgetTokensSpent != 0) {
                            if (totalValue + TFHE.decrypt(option.budgetTokensSpent) > settings.budgetAmount) break; // break outer loop for non-ETH tokens
                            totalValue += TFHE.decrypt(option.budgetTokensSpent);
                        }
                    }
                }
        ...
        calldatas[executeParamsLength] = abi.encodeWithSelector(
            this._afterExecute.selector,
            proposalId,
            proposalData,
            totalValue
        );
    ...
    function _voteSucceeded(uint256 proposalId) external view override returns (bool) {
        ...
        unchecked {
            if (proposal.settings.criteria == uint8(PassingCriteria.Threshold)) {
                for (uint256 i; i < n; ++i) {
                    if (TFHE.decrypt(TFHE.ge(proposal.optionVotes[i], TFHE.asEuint32(proposal.settings.criteriaValue))))
                        return true;
                }
            } else if (proposal.settings.criteria == uint8(PassingCriteria.TopChoices)) {
                for (uint256 i; i < n; ++i) {
                    if (TFHE.decrypt(TFHE.gt(proposal.optionVotes[i], TFHE.asEuint32(0)))) return true;
                }
            }
        }
    ...
    function _recordVote(
        ...
        for (uint256 i; i < totalOptions; ) {
            ...
            proposals[proposalId].optionVotes[option] = TFHE.add(proposals[proposalId].optionVotes[option], weight);
    ...
    function _sortOptions(
        euint32[] memory optionVotes,
        ProposalOption[] memory options
    ) internal view returns (euint32[] memory, ProposalOption[] memory) {
        unchecked {
            euint32 highestValue;
            ...
            for (uint256 i; i < optionVotes.length - 1; ++i) {
                ...
                for (uint256 j = i + 1; j < optionVotes.length; ++j) {
                    if (TFHE.decrypt(TFHE.gt(optionVotes[j], highestValue))) {
    ...
    ) internal view returns (uint256 executeParamsLength, uint256 succeededOptionsLength) {
        uint256 n = options.length;
        unchecked {
            uint256 i;
            if (settings.criteria == uint8(PassingCriteria.Threshold)) {
                ...
                for (i; i < n; ++i) {
                    if (TFHE.decrypt(TFHE.ge(optionVotes[i], TFHE.asEuint32(settings.criteriaValue)))) {
                ...
                for (i; i < settings.criteriaValue; ++i) {
                    if (TFHE.decrypt(TFHE.gt(optionVotes[i], TFHE.asEuint32(0)))) {
                        executeParamsLength += options[i].targets.length;
                    } else {
                        break;
                    }
    ...
    // Add a function to reveal the tally (this should be carefully controlled in a real-world scenario)
    function revealTally(uint256 proposalId) external onlyGovernor {
        Proposal storage proposal = proposals[proposalId];
        uint32[] memory decryptedVotes = new uint32[](proposal.optionVotes.length);
        for (uint256 i = 0; i < proposal.optionVotes.length; i++) {
            decryptedVotes[i] = TFHE.decrypt(proposal.optionVotes[i]);
        }
        emit TallyRevealed(proposalId, decryptedVotes);
    }

    event TallyRevealed(uint256 indexed proposalId, uint32[] votes);
}
